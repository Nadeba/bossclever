                      ...
                      <div style={{ borderTop: "1px solid #E5E3DA" }}>
                        {g.taches.map((t) => (
                          ...
                          >
                            ...
                            {t.statut !== "validee" && (estPrincipal || peutFaire("validerTaches")) && (
                              <Bouton
                                variant="danger"
                                onClick={() => {
                                  if (window.confirm(`Retirer définitivement la tâche « ${t.titre} » ?`)) {
                                    retirerTache(g.empId, t.id);
                                  }
                                }}
                              >
                                Retirer
...
export default function App() {
  ...
  // Permet au manager de retirer définitivement une tâche assignée à un
  // employé (jusqu'ici, aucune tâche ne pouvait être supprimée une fois
  // créée). Volontairement limité aux tâches non encore validées côté
  // interface, pour ne jamais toucher au système de points déjà attribué.
  const retirerTache = (empId, tacheId) => {
    if (gestionGelee) return;
    majEmploye(empId, (e) => {
      const tache = e.taches.find((t) => t.id === tacheId);
      enregistrerActivite("Suppression de tâche", `« ${tache?.titre || ""} » pour ${e.nom}`);
      return {
        ...e,
