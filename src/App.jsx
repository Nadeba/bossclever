...
function VueManager({ employes, valider, rejeter, assigner, criteres, setCriteres }) {
...
function VueEmploye({ employe, pointer, declarer, majBilan, noterCollegue, autres, criteres }) {
...
async function chargerDepuisSupabase() {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/etat_app?id=eq.1&select=donnees`, {
    headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
  });
  const data = await res.json();
  return data && data[0] ? data[0].donnees : null;
}

async function sauvegarderDansSupabase(employes) {
  await fetch(`${SUPABASE_URL}/rest/v1/etat_app?id=eq.1`, {
    method: "PATCH",
    headers: {
      apikey: SUPABASE_KEY,
