  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: "0 10px 30px rgba(15, 47, 82, 0.055)",
        ...style,
      }}
    >
      {children}
...
function Bouton({ children, onClick, variant = "secondary", style, disabled }) {
  ...
  return (
    <button
      className={`bc-button bc-button-${variant}`}
      onClick={onClick}
      disabled={disabled}
      style={{
        ...variants[variant],
...
const STYLES_PREMIUM_GLOBAL = `
  :root {
    ...
    --bc-muted: #667085;
    --bc-border: #E7ECF3;
    --bc-shadow: 0 14px 38px rgba(16, 47, 82, .08);
  }
  html { scroll-behavior: smooth; }
  body {
    background:
      radial-gradient(circle at 7% 5%, rgba(255,217,59,.10), transparent 22%),
      radial-gradient(circle at 96% 2%, rgba(24,119,242,.10), transparent 25%),
      #F7F9FC !important;
    color: var(--bc-text);
  }
  * { box-sizing: border-box; }
  ::selection { background: rgba(24,119,242,.18); color: #102F52; }
  .bc-card {
    position: relative;
    overflow: hidden;
  ...
  .bc-card:hover {
    ...
    box-shadow: 0 18px 45px rgba(16,47,82,.10) !important;
  ...
  .bc-card::after {
    ...
    border-radius: 50%;
    background: radial-gradient(circle, rgba(24,119,242,.07), transparent 68%);
    right: -34px; top: -38px;
    pointer-events: none;
  }
  .bc-button {
    ...
    box-shadow: 0 6px 16px rgba(16,47,82,.06);
  }
  .bc-button:not(:disabled):hover { transform: translateY(-1px); filter: brightness(.985); }
  .bc-button-primary:not(:disabled):hover { box-shadow: 0 10px 24px rgba(24,119,242,.28); }
  .bc-button-success:not(:disabled):hover { box-shadow: 0 10px 24px rgba(15,110,86,.20); }
  .bc-button:active:not(:disabled) { transform: translateY(0) scale(.985); }
  .bc-badge { display: inline-flex; align-items: center; gap: 5px; border: 1px solid rgba(16,47,82,.04); }
  .bc-avatar { box-shadow: inset 0 0 0 1px rgba(16,47,82,.06), 0 4px 12px rgba(16,47,82,.07); }
  .bc-alert { animation: bcAlertIn .32s ease both; box-shadow: 0 6px 18px rgba(16,47,82,.04); }
  ...
  input:hover, select:hover, textarea:hover { border-color: #C8D7EA !important; }
  input:focus, select:focus, textarea:focus {
    border-color: #1877F2 !important;
    box-shadow: 0 0 0 3px rgba(24,119,242,.11) !important;
    background: #fff !important;
  }
  table { border-collapse: separate; border-spacing: 0; width: 100%; }
  th { color: #64748B !important; font-size: 11.5px !important; text-transform: uppercase; letter-spacing: .045em; }
  tbody tr { transition: background .16s ease, transform .16s ease; }
  tbody tr:hover { background: #F8FBFF; }
  a, [role="button"] { transition: color .18s ease, opacity .18s ease; }
  h1, h2, h3 { letter-spacing: -.025em; }
  .nav-panel-inner { animation: bcPanelIn .34s cubic-bezier(.2,.8,.2,1) both; }
  .rj-panel-inner { animation: bcPanelIn .3s ease both; }
  .stat-illu { animation: bcFloat 4.5s ease-in-out infinite; transform-origin: center; }
  .stat-line { stroke-dasharray: 900; stroke-dashoffset: 900; animation: bcDraw 1.2s ease forwards; }
  .stat-area { animation: bcFadeUp .8s ease both; }
  .stat-dot { animation: bcPop .5s ease both; transform-origin: center; }
  .nav-shell { align-items: flex-start; gap: 22px !important; }
  .nav-content { animation: bcFadeUp .38s ease both; min-width: 0; }
  .nav-content > div[style*="margin-bottom: 16px"] {
    background: rgba(255,255,255,.75); border: 1px solid #E7ECF3; border-radius: 12px; padding: 9px 12px;
  }
  .rj-card { border-radius: 18px !important; border-color: #E7ECF3 !important; box-shadow: var(--bc-shadow); background: rgba(255,255,255,.96) !important; }
  .rj-tabs { background: #F5F8FC !important; border-bottom-color: #E7ECF3 !important; }
  .rj-tab { transition: color .18s ease, background .18s ease, transform .18s ease !important; }
  .rj-tab:hover { color: #1877F2 !important; background: #EFF6FF !important; }
  .rj-tab.active { color: #0F57C8 !important; font-weight: 700 !important; }
  .rj-row { border-radius: 11px; transition: background .18s ease, transform .18s ease; }
  .rj-row:hover { background: #F8FBFF; transform: translateX(2px); }
  @keyframes bcPanelIn { from { opacity:0; transform: translateY(10px); } to { opacity:1; transform:none; } }
  @keyframes bcFadeUp { from { opacity:0; transform: translateY(8px); } to { opacity:1; transform:none; } }
  @keyframes bcAlertIn { from { opacity:0; transform: translateX(-6px); } to { opacity:1; transform:none; } }
  @keyframes bcFloat { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-4px); } }
  @keyframes bcDraw { to { stroke-dashoffset: 0; } }
  @keyframes bcPop { from { opacity:0; transform: scale(.6); } to { opacity:1; transform: scale(1); } }
  @media (prefers-reduced-motion: reduce) {
    *, *::before, *::after { animation-duration: .01ms !important; animation-iteration-count: 1 !important; transition-duration: .01ms !important; }
  }
  @media (max-width: 760px) {
    .bc-card { border-radius: 15px !important; }
    .bc-button { min-height: 40px; }
  }
`;
...
const STYLES_NAV = `
  ...
  .nav-sidebar {
    ...
    background: rgba(255,255,255,.88); border: 1px solid #E7ECF3; border-radius: 18px;
    padding: 10px; position: sticky; top: 16px;
    box-shadow: 0 14px 35px rgba(16,47,82,.07); backdrop-filter: blur(14px);
  }
  .nav-tab-v {
    display: flex; align-items: center; gap: 10px; padding: 10px 12px;
    border: none; background: transparent; cursor: pointer; text-align: left;
    font-size: 13.5px; color: #667085; font-family: inherit; border-radius: 11px;
    border-left: 3px solid transparent; font-weight: 600;
    transition: background .2s ease, color .2s ease, transform .2s ease, box-shadow .2s ease;
  }
  .nav-tab-v:hover { background: #F1F6FD; color: #102F52; transform: translateX(2px); }
  .nav-tab-v.active {
    background: linear-gradient(135deg, #1877F2, #0F57C8); color: #fff; font-weight: 700;
    border-left: 3px solid #FFD93B; box-shadow: 0 8px 18px rgba(24,119,242,.20);
  }
  .nav-tab-v .n-count {
    margin-left: auto; font-size: 10.5px; background: #FFF4BF; color: #765A00;
    border-radius: 20px; padding: 2px 7px; font-weight: 800;
  }
  .nav-tab-v.active .n-count { background: #FFD93B; color: #5C4600; }
  .nav-content { flex: 1; min-width: 0; }
  .nav-sidebar-manager {
    background: linear-gradient(180deg, #092C66 0%, #061F4B 62%, #041737 100%);
    border: 1px solid rgba(255,255,255,.08);
    box-shadow: 0 24px 55px rgba(4,31,75,.22);
    padding: 14px 11px;
  }
  .nav-sidebar-manager::before {
    content: "BossClever";
    color: #fff; font-size: 18px; font-weight: 850; letter-spacing: -.03em;
    padding: 8px 10px 15px; margin-bottom: 5px; border-bottom: 1px solid rgba(255,255,255,.10);
  }
  .nav-sidebar-manager .nav-tab-v { color: rgba(255,255,255,.76); border-left-color: transparent; }
  .nav-sidebar-manager .nav-tab-v:hover { background: rgba(255,255,255,.08); color: #fff; transform: translateX(3px); }
  .nav-sidebar-manager .nav-tab-v.active {
    background: linear-gradient(135deg, #1D74EF, #1558C8); color: #fff;
    border-left-color: #FFD93B; box-shadow: 0 12px 26px rgba(0,0,0,.20);
  }
  .nav-sidebar-manager .nav-tab-v .n-count { background: rgba(255,217,59,.95); color: #4A3900; }
  .bc-manager-header {
    display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap;
    background:linear-gradient(135deg,rgba(255,255,255,.95),rgba(247,251,255,.92));
    border:1px solid #E6EDF7; border-radius:20px; padding:20px 22px;
    box-shadow:0 14px 38px rgba(16,47,82,.055); margin-bottom:20px; overflow:hidden; position:relative;
  }
  .bc-manager-header::after { content:""; position:absolute; width:180px; height:180px; right:-70px; top:-90px; border-radius:50%; background:radial-gradient(circle,rgba(24,119,242,.12),transparent 68%); }
  .bc-manager-greeting { margin:0; font-size:28px; font-weight:850; color:#0B2850; line-height:1.05; letter-spacing:-.045em; }
  .bc-manager-subtitle { margin:7px 0 0; color:#6A778A; font-size:13.5px; }
  .bc-manager-date { display:flex; align-items:center; gap:9px; background:#fff; border:1px solid #E4EAF3; border-radius:12px; padding:9px 12px; color:#244263; font-size:12.5px; font-weight:700; box-shadow:0 7px 18px rgba(16,47,82,.05); z-index:1; }
  .bc-kpi-grid { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:14px; }
  .bc-kpi { background:#fff; border:1px solid #E5ECF6; border-radius:18px; padding:17px; box-shadow:0 12px 32px rgba(16,47,82,.055); transition:transform .22s ease, box-shadow .22s ease; position:relative; overflow:hidden; }
  .bc-kpi:hover { transform:translateY(-3px); box-shadow:0 18px 42px rgba(16,47,82,.10); }
  .bc-kpi-top { display:flex; justify-content:space-between; gap:10px; align-items:flex-start; }
  .bc-kpi-icon { width:42px; height:42px; border-radius:13px; display:flex; align-items:center; justify-content:center; font-size:20px; }
  .bc-kpi-label { margin:12px 0 3px; color:#69778B; font-size:12.5px; font-weight:650; }
  .bc-kpi-value { margin:0; font-size:29px; font-weight:850; color:#0B2850; letter-spacing:-.04em; }
  .bc-kpi-foot { margin-top:10px; color:#708096; font-size:11.5px; display:flex; align-items:center; gap:5px; }
  .bc-dashboard-grid { display:grid; grid-template-columns:minmax(0,1.18fr) minmax(320px,.82fr); gap:16px; }
  .bc-dashboard-lower { display:grid; grid-template-columns:1.05fr 1fr 1fr; gap:16px; }
  .bc-premium-panel { background:#fff; border:1px solid #E5ECF6; border-radius:20px; padding:19px; box-shadow:0 13px 34px rgba(16,47,82,.055); }
  .bc-panel-head { display:flex; justify-content:space-between; align-items:center; gap:12px; margin-bottom:15px; }
  .bc-panel-title { margin:0; font-size:15px; font-weight:800; color:#102F52; }
  .bc-panel-link { border:0; background:none; color:#1877F2; font-size:12px; font-weight:750; cursor:pointer; padding:3px; }
  .bc-health-hero { background:linear-gradient(135deg,#07295E,#031C43 70%); color:#fff; position:relative; overflow:hidden; }
  .bc-health-hero::after { content:""; position:absolute; width:240px; height:240px; border-radius:50%; right:-95px; bottom:-135px; background:radial-gradient(circle,rgba(24,119,242,.60),transparent 65%); }
  .bc-progress-track { height:7px; background:#EDF1F6; border-radius:999px; overflow:hidden; }
  .bc-progress-fill { height:100%; border-radius:999px; background:linear-gradient(90deg,#1877F2,#4E95F4); animation:bcGrow .75s cubic-bezier(.2,.8,.2,1) both; transform-origin:left; }
  .bc-rank-row { display:flex; align-items:center; gap:10px; padding:10px 0; border-bottom:1px solid #EEF2F7; }
  .bc-rank-row:last-child { border-bottom:0; padding-bottom:0; }
  .bc-rank-score { margin-left:auto; font-weight:850; color:#102F52; font-size:16px; }
  .bc-activity-row { display:flex; gap:10px; padding:10px 0; border-bottom:1px solid #EEF2F7; }
  .bc-activity-row:last-child { border-bottom:0; }
  .bc-activity-dot { width:30px; height:30px; border-radius:10px; background:#EDF5FF; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
  .bc-mini-bars { display:flex; align-items:flex-end; gap:7px; height:105px; padding-top:10px; }
  .bc-mini-bar { flex:1; min-width:13px; border-radius:7px 7px 3px 3px; background:linear-gradient(180deg,#448DF1,#1877F2); animation:bcBarIn .65s ease both; transform-origin:bottom; }
  @keyframes bcGrow { from { transform:scaleX(0); } to { transform:scaleX(1); } }
  @keyframes bcBarIn { from { transform:scaleY(.05); opacity:.2; } to { transform:scaleY(1); opacity:1; } }
  @media (max-width: 1080px) { .bc-kpi-grid { grid-template-columns:repeat(2,minmax(0,1fr)); } .bc-dashboard-lower { grid-template-columns:1fr 1fr; } }
  @media (max-width: 820px) { .bc-dashboard-grid,.bc-dashboard-lower { grid-template-columns:1fr; } }
  @media (max-width: 760px) {
...
function OngletsNav({ onglets, actif, onChange, variant = "default" }) {
  return (
    <div className={`nav-sidebar ${variant === "manager" ? "nav-sidebar-manager" : ""}`}>
      {onglets.map((o) => (
        <button
          key={o.id}
...
function VueManager({
  ...
  const managerDashboard = useMemo(() => {
    const toutes = employes.flatMap((e) => e.taches || []);
    const validees = toutes.filter((t) => t.statut === "validee").length;
    const enCours = toutes.filter((t) => t.statut === "a_faire").length;
    const validation = toutes.filter((t) => t.statut === "en_attente").length;
    const objectifs = employes.flatMap((e) => e.objectifs || []);
    const objectifsAtteints = objectifs.filter((o) => {
      const emp = employes.find((e) => (e.objectifs || []).some((x) => x.id === o.id));
      return emp ? statutObjectif(o, emp.taches || []) === "atteint" : false;
    }).length;
    const objectifPct = objectifs.length ? Math.round((objectifsAtteints / objectifs.length) * 100) : 0;
    const performance = classement.length ? Math.round(classement.reduce((s,e)=>s+e.score,0)/classement.length) : 0;
    return {
      totalTaches: toutes.length, validees, enCours, validation,
      objectifsTotal: objectifs.length, objectifsAtteints, objectifPct, performance,
    };
  }, [employes, classement]);

  const nomManagerAffiche = monIdentite?.estPrincipal
    ? "Manager"
    : `${monIdentite?.prenom || ""} ${monIdentite?.nom || ""}`.trim() || "Manager";

  const dateDashboard = new Date().toLocaleDateString("fr-FR", {
  ...
  const soumettreTache = () => {
    ...
    // Le délai est obligatoire et ne peut pas être antérieur à aujourd'hui.
    if (!titreTache.trim() || !empChoisi || !echeanceTache) return;
    if (echeanceTache < aujourdHui) return;
  ...
  return (
    <div className="nav-shell">
      ...
      <OngletsNav onglets={onglets} actif={ongletActif} onChange={setOngletActif} variant="manager" />

      <div className="nav-content">
      <div className="bc-manager-header">
        <div style={{ position: "relative", zIndex: 1 }}>
          <h1 className="bc-manager-greeting">Bonjour {nomManagerAffiche} 👋</h1>
          <p className="bc-manager-subtitle">Voici ce qui se passe aujourd'hui dans votre équipe.</p>
        </div>
        <div className="bc-manager-date">📅 {dateDashboard}</div>
      </div>
        ...
        {ongletActif === "apercu" && (
          <>
            ...
            <div className="bc-kpi-grid">
              {[
                { label: "Employés", value: employes.length, foot: `${presentsAujourdhui.length} présent${presentsAujourdhui.length > 1 ? "s" : ""} aujourd'hui`, icon: "👥", bg: "#EAF3FF" },
                { label: "Tâches terminées", value: managerDashboard.validees, foot: `${managerDashboard.validation} en attente de validation`, icon: "✓", bg: "#E8F8EF" },
                { label: "Objectifs atteints", value: `${managerDashboard.objectifPct}%`, foot: `${managerDashboard.objectifsAtteints}/${managerDashboard.objectifsTotal || 0} objectifs`, icon: "◎", bg: "#FFF5CF" },
                { label: "Performance équipe", value: `${managerDashboard.performance}%`, foot: "Score moyen calculé en direct", icon: "↗", bg: "#F0ECFF" },
              ].map((k) => (
                <div className="bc-kpi" key={k.label}>
                  <div className="bc-kpi-top">
                    <div className="bc-kpi-icon" style={{ background: k.bg }}>{k.icon}</div>
                    <span style={{ color: "#16A34A", fontSize: 11.5, fontWeight: 800 }}>● Live</span>
                  </div>
                  <p className="bc-kpi-label">{k.label}</p>
                  <p className="bc-kpi-value">{k.value}</p>
                  <div className="bc-kpi-foot"><span style={{ color: "#16A34A" }}>↑</span>{k.foot}</div>
                </div>
              ))}
            </div>

            <div className="bc-dashboard-grid">
              <div className="bc-premium-panel bc-health-hero">
                <div className="bc-panel-head" style={{ position: "relative", zIndex: 1 }}>
                  <div>
                    <p style={{ margin: 0, color: "#8DBBFF", fontSize: 11, fontWeight: 850, textTransform: "uppercase", letterSpacing: ".08em" }}>Tableau de bord intelligent</p>
                    <h3 style={{ margin: "5px 0 0", fontSize: 22, color: "#fff", fontWeight: 850, letterSpacing: "-.035em" }}>Santé de l'entreprise</h3>
                  </div>
                  <div style={{ width: 74, height: 74, borderRadius: "50%", background: `conic-gradient(#FFD93B ${healthScore.global * 3.6}deg, rgba(255,255,255,.13) 0deg)`, display: "grid", placeItems: "center" }}>
                    <div style={{ width: 58, height: 58, borderRadius: "50%", background: "#09295B", display: "grid", placeItems: "center", fontSize: 19, fontWeight: 900 }}>{healthScore.global}</div>
                  </div>
                </div>
                <p style={{ margin: "0 0 18px", maxWidth: 620, color: "rgba(255,255,255,.72)", fontSize: 13, lineHeight: 1.6, position: "relative", zIndex: 1 }}>
                  Une lecture instantanée de la présence, de la performance, des délais, des rapports et de la collaboration.
                </p>
                <div style={{ display: "grid", gap: 10, position: "relative", zIndex: 1 }}>
                  {healthScore.dimensions.map((d) => (
                    <div key={d.label}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11.5, marginBottom: 5 }}>
                        <span style={{ color: "rgba(255,255,255,.75)" }}>{d.label}</span><strong>{d.valeur}%</strong>
                      </div>
                      <div style={{ height: 6, background: "rgba(255,255,255,.12)", borderRadius: 999, overflow: "hidden" }}>
                        <div className="bc-progress-fill" style={{ width: `${d.valeur}%`, background: d.label === "Respect des délais" ? "#FFD93B" : "linear-gradient(90deg,#2F80ED,#72ACFF)" }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bc-premium-panel">
                <div className="bc-panel-head">
                  <h3 className="bc-panel-title">Activité & alertes</h3>
                  <button className="bc-panel-link" onClick={() => setOngletActif("taches")}>Voir les tâches →</button>
                </div>
                <div>
                  <div className="bc-activity-row"><div className="bc-activity-dot">📍</div><div><strong style={{ fontSize: 12.5, color: "#102F52" }}>{radarDirigeant.presents}/{radarDirigeant.total} ont pointé</strong><div style={{ fontSize: 11.5, color: "#7B8798", marginTop: 2 }}>Présence enregistrée aujourd'hui</div></div></div>
                  <div className="bc-activity-row"><div className="bc-activity-dot">⏰</div><div><strong style={{ fontSize: 12.5, color: "#102F52" }}>{radarDirigeant.retardsSemaine} retard(s)</strong><div style={{ fontSize: 11.5, color: "#7B8798", marginTop: 2 }}>Signalés cette semaine</div></div></div>
                  <div className="bc-activity-row"><div className="bc-activity-dot">✅</div><div><strong style={{ fontSize: 12.5, color: "#102F52" }}>{enAttente.length} validation(s)</strong><div style={{ fontSize: 11.5, color: "#7B8798", marginTop: 2 }}>Tâches déclarées accomplies</div></div></div>
                  <div className="bc-activity-row"><div className="bc-activity-dot">🏆</div><div><strong style={{ fontSize: 12.5, color: "#102F52" }}>{radarDirigeant.deptLeader?.dept || "—"}</strong><div style={{ fontSize: 11.5, color: "#7B8798", marginTop: 2 }}>Département actuellement en tête</div></div></div>
                </div>
              </div>
            </div>

            {(enAttente.length > 0 || contestationsOuvertes.length > 0 || anomaliesEmployes.length > 0 || anomaliesDepartements.length > 0) && (
              <div style={{ display: "grid", gap: 8 }}>
                {enAttente.length > 0 && <Alerte>🔔 {enAttente.length} tâche{enAttente.length > 1 ? "s" : ""} en attente de votre validation.</Alerte>}
                {contestationsOuvertes.length > 0 && <Alerte tone="accent">⚖️ {contestationsOuvertes.length} contestation{contestationsOuvertes.length > 1 ? "s" : ""} à traiter.</Alerte>}
                {anomaliesEmployes.slice(0, 2).map((a) => <Alerte key={a.nom} tone="accent">⚠️ La performance de {a.nom} a diminué de {a.baisse}% sur quatre semaines.</Alerte>)}
                {anomaliesDepartements.slice(0, 1).map((a) => <Alerte key={a.dept} tone="accent">⚠️ Le département {a.dept} connaît une baisse d'activité de {a.baisse}%.</Alerte>)}
              </div>
            )}

            <div className="bc-dashboard-lower">
              <div className="bc-premium-panel">
                <div className="bc-panel-head"><h3 className="bc-panel-title">Performance de l'équipe</h3><button className="bc-panel-link" onClick={() => setOngletActif("statistiques")}>Voir le rapport →</button></div>
                <GraphiqueEvolution points={statsAnnee.total || []} moisNoms={statsAnnee.moisNoms || []} moisCourant={new Date().getMonth()} hauteur={125} />
              </div>

              <div className="bc-premium-panel">
                <div className="bc-panel-head"><h3 className="bc-panel-title">Objectifs & exécution</h3><button className="bc-panel-link" onClick={() => setOngletActif("taches")}>Détail →</button></div>
                <div style={{ display: "grid", gap: 14 }}>
                  {[
                    ["Objectifs atteints", managerDashboard.objectifPct, "#16A34A"],
                    ["Tâches validées", managerDashboard.totalTaches ? Math.round(managerDashboard.validees / managerDashboard.totalTaches * 100) : 0, "#1877F2"],
                    ["Présence du jour", employes.length ? Math.round(presentsAujourdhui.length / employes.length * 100) : 0, "#F0B90B"],
                  ].map(([label, pct, color]) => (
                    <div key={label}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 12.5 }}><span style={{ color: "#44546A" }}>{label}</span><strong style={{ color }}>{pct}%</strong></div>
                      <div className="bc-progress-track"><div className="bc-progress-fill" style={{ width: `${pct}%`, background: color }} /></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bc-premium-panel">
                <div className="bc-panel-head"><h3 className="bc-panel-title">Répartition des tâches</h3><button className="bc-panel-link" onClick={() => setOngletActif("taches")}>Voir tout →</button></div>
                <div className="bc-mini-bars">
                  {[
                    [managerDashboard.validees, "Terminées"],
                    [managerDashboard.enCours, "En cours"],
                    [managerDashboard.validation, "Validation"],
                  ].map(([v,l],i) => {
                    const max = Math.max(1, managerDashboard.validees, managerDashboard.enCours, managerDashboard.validation);
                    const h = Math.max(12, Math.round((v / max) * 92));
                    return <div key={l} style={{ flex:1, textAlign:"center" }}><div className="bc-mini-bar" style={{ height:h, animationDelay:`${i*.12}s`, background:i===1?"linear-gradient(180deg,#8BB8F6,#4D8FEA)":i===2?"linear-gradient(180deg,#FFD965,#F1B800)":"linear-gradient(180deg,#448DF1,#1877F2)" }} /><div style={{ fontSize:11, fontWeight:800, color:"#102F52", marginTop:7 }}>{v}</div><div style={{ fontSize:9.8, color:"#7B8798", marginTop:2 }}>{l}</div></div>
                  })}
                </div>
              </div>
            </div>
            ...
            <div className="bc-dashboard-grid">
              ...
              <div className="bc-premium-panel">
                <div className="bc-panel-head"><h3 className="bc-panel-title">Classement en direct</h3><Bouton variant="secondary" onClick={() => exporterPDF(classement, criteres)}>↓ PDF</Bouton></div>
                <div>
                  {classement.slice(0, 5).map((e, i) => (
                    <div className="bc-rank-row" key={e.id}>
                      <span style={{ width: 22, textAlign: "center" }}>{i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : i + 1}</span>
                      <Avatar initiales={e.initiales} tone={i === 0 ? "success" : "neutral"} />
                      <div style={{ minWidth: 0 }}><div style={{ fontSize: 12.5, fontWeight: 750, color: "#243B57", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{e.nom}</div><div style={{ fontSize: 10.5, color: "#8490A1", marginTop: 2 }}>{e.dept}</div></div>
                      <div className="bc-rank-score">{e.score}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {parDepartement.length > 1 && (
              <div className="bc-premium-panel">
                <div className="bc-panel-head"><h3 className="bc-panel-title">Comparaison par département</h3><span style={{ fontSize: 11.5, color: "#7B8798" }}>Moyenne sur 100</span></div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 12 }}>
                  {parDepartement.map((g) => (
                    <div key={g.dept} style={{ border: "1px solid #E8EEF6", borderRadius: 14, padding: 13, background: "#FBFDFF" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><div><div style={{ fontSize: 12.5, fontWeight: 800, color: "#102F52" }}>{g.dept}</div><div style={{ fontSize: 10.5, color: "#8490A1", marginTop: 2 }}>{g.employes.length} employé{g.employes.length > 1 ? "s" : ""}</div></div><strong style={{ fontSize: 18, color: "#1877F2" }}>{g.moyenne}</strong></div>
                      <div className="bc-progress-track" style={{ marginTop: 11 }}><div className="bc-progress-fill" style={{ width: `${Math.max(0,Math.min(100,g.moyenne))}%` }} /></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {(estPrincipal || peutFaire("assignerTaches")) && (
              <div>
                ...
                <Card style={{ display: "grid", gap: 10 }}>
                  ...
                  <Bouton
                    variant="primary"
                    onClick={soumettreTache}
                    disabled={
                      !titreTache.trim() ||
                      !echeanceTache ||
                      echeanceTache < dateJourISO()
                    }
                  >
                    + Ajouter la tâche
                  </Bouton>
...
function envoyerNotification(destinataire, sujet, titre, contenuHtml) {
  if (!destinataire) return;
  fetch("/api/envoyer-email", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ destinataire, sujet, corpsHtml: templateEmail(titre, contenuHtml) }),
  }).catch(() => {});
}
...
export default function App() {
  useEffect(() => {
    ...
    style.id = id;
    style.textContent = STYLES_PREMIUM_GLOBAL;
    document.head.appendChild(style);
    return () => {
      const actuel = document.getElementById(id);
