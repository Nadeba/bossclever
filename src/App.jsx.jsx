...
async function chargerDepuisSupabase(accessToken) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/etat_app?select=id,donnees,owner_id`, {
    headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${accessToken}` },
  });
  const data = await res.json();
...
async function lierEmploye(accessToken, email) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/lier_employe`, {
    ...
    body: JSON.stringify({ mon_email: email }),
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!ligne) {
          // Aucune fiche accessible : on tente de se lier automatiquement
          // en tant qu'employé via l'email utilisé à l'inscription.
          const lien = await lierEmploye(session.access_token, session.email);
