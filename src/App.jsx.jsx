      ...
      if (mode === "inscription") {
        ...
        if (codeSaisi) {
          localStorage.setItem(
            "bc_code_attente",
            JSON.stringify({ email, code: codeSaisi })
          );
        }
        setMode("connexion");
    ...
    >
      ...
      {mode === "inscription" && (
        <p style={{ fontSize: 12, color: "#5F5E5A", margin: "-8px 0 14px" }}>
          Employé : utilisez l'adresse email exacte transmise par votre manager.
        </p>
      )}
      ...
      {mode === "inscription" && (
        <>
          ...
          <label style={{ fontSize: 13, color: "#5F5E5A", display: "block", marginBottom: 4 }}>
            Code entreprise (employé uniquement)
          </label>
          <input
            type="text"
      ...
      try {
        ...
        if (!ligne) {
          ...
          if (attente && attente.email === session.email && attente.code) {
            ...
            if (lien) ligne = await chargerDepuisSupabase(session.access_token);
          }
          localStorage.removeItem("bc_code_attente");
        }

        if (!ligne && codeEtaitFourni) {
          // Un code d'entreprise avait été fourni mais la liaison a
          // échoué (email non ajouté par le manager) : on bloque l'accès
    ...
    return (
      ...
      >
        ...
        <p style={{ fontSize: 14, color: "#5F5E5A", margin: "0 0 20px" }}>
          Votre email n'a pas été ajouté par cette entreprise. Contactez votre manager pour
          qu'il vous ajoute avant de réessayer.
        </p>
        <button
          onClick={() => {
