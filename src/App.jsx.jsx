...
function InitiativeAValiderCard({ initiative, valider, rejeter }) {
  const [commentaire, setCommentaire] = useState("");
  return (
    <Card style={{ padding: "12px 16px" }}>
      ...
      />
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
        <Bouton variant="danger" onClick={() => rejeter(initiative.empId, initiative.id, commentaire)}>
          Ne pas retenir
        </Bouton>
        <Bouton variant="success" onClick={() => valider(initiative.empId, initiative.id, commentaire)}>
          Valider (+10 pts)
        </Bouton>
      </div>
    </Card>
...
function calculerArchives(employes, periode, heureArrivee) {
  return employes.map((e) => {
    ...
    const joursRetard = pointagesPeriode.filter(
      (p) => p.arrivee && heureArrivee && p.arrivee > heureArrivee
    ).length;
    ...
    <div style={{ display: "grid", gap: 18 }}>
      ...
      >
        {[
          ...
          { label: "Retards à vie", valeur: employe.retardsTotalVie || 0 },
  ...
  const presenceAujourdhui = useMemo(
    () =>
      employes.map((e) => {
        ...
        const enRetard = !!(arrivee && heureArrivee && arrivee > heureArrivee);
        const departAnticipe = !!(depart && heureDepart && depart < heureDepart);
        return { ...e, arrivee, depart, enRetard, departAnticipe };
      }),
            ...
            <div className="stat-chart-wrap">
              <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "center" }}>
                ...
                >
                  ...
                  >
                    <span style={{ fontSize: 24, fontWeight: 700 }}>{healthScore.global}</span>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    ...
                    {statsAnnee.penteTotal !== 0 && (
                      <Badge tone={statsAnnee.penteTotal > 0 ? "success" : "warning"}>
                        {statsAnnee.penteTotal > 0 ? "↑ tendance positive ce mois-ci" : "↓ tendance en baisse ce mois-ci"}
                      </Badge>
                    )}
                  </div>
                  <div style={{ display: "grid", gap: 6 }}>
                    {healthScore.dimensions.map((d) => (
                      <div key={d.label} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
                        <span style={{ width: 120, color: "#5F5E5A" }}>{d.label}</span>
          ...
          <>
            ...
            {estPrincipal && (
              <div>
                ...
                <p style={{ margin: "0 0 12px", fontSize: 13, color: "#5F5E5A" }}>
                  Sert à déterminer automatiquement les retards et départs anticipés dans l'onglet
                  "Présence". Propre à votre entreprise — modifiable à tout moment.
                </p>
          ...
          <>
            ...
            <Card>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  {pointageAujourdhui?.arrivee && (
                    <Badge tone="success">Arrivée à {pointageAujourdhui.arrivee}</Badge>
                  )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    sauvegarderDansSupabase(session.access_token, rowId, { employes, criteres, logoUrl, bureau, plan, heureArrivee, heureDepart }).catch(() => {});
  }, [employes, criteres, logoUrl, bureau, plan, heureArrivee, heureDepart, chargement, session, rowId]);
  ...
  const enregistrerPointage = (id, champ, heure) =>
    majEmploye(id, (e) => {
      const liste = e.historiquePointage || [];
      const d = dateJourISO();
      const idx = liste.findIndex((p) => p.date === d);
      if (idx >= 0) {
        const maj = { ...liste[idx], [champ]: heure };
        return { ...e, historiquePointage: liste.map((p, i) => (i === idx ? maj : p)) };
      }
      return { ...e, historiquePointage: [...liste, { date: d, [champ]: heure }] };
    });
  ...
  const pointer = async (id) => {
    ...
    if (!bureauConfigure) {
      enregistrerPointage(id, "arrivee", h);
      return { ok: true };
    }
  ...
  const validerBilan = (empId) =>
    majEmploye(empId, (e) => {
      ...
      return {
        ...e,
        journalPoints: [...(e.journalPoints || []), ...nouveauxPoints],
        retardsTotalVie: (e.retardsTotalVie || 0) + e.retards,
        retards: 0,
      };
    });
