...
function GestionAbonnement({ planActuel, onChoisir }) {
  return (
    <div>
      <h3 style={{ fontSize: 16, fontWeight: 500, margin: "0 0 4px" }}>Abonnement</h3>
      <p style={{ margin: "0 0 12px", fontSize: 13, color: "#5F5E5A" }}>
        Aucun paiement n'est encore prélevé pour l'instant — la facturation en ligne sera activée
        prochainement.
      </p>
      <div
        style={{
          ...
          return (
            ...
            >
              ...
              {actif ? (
                ...
                <Bouton variant={p.populaire ? "primary" : "secondary"} onClick={() => onChoisir(p.id)}>
                  Choisir ce plan
                </Bouton>
              )}
            </Card>
      ...
      <div className="nav-panel-inner" key={ongletActif} style={{ display: "grid", gap: 24 }}>
        ...
        {ongletActif === "compte" && (
          <>
            ...
            {estPrincipal && <GestionAbonnement planActuel={plan} onChoisir={changerPlan} />}
          </>
        )}
      </div>
    </div>
