...
function calculerStatsPlateforme(lignes) {
  const entreprises = lignes.map((ligne) => {
    ...
    return {
      ...
      createdAt: ligne.created_at || null,
  ...
  const mrr = entreprises.reduce((somme, e) => {
    ...
    return somme + (infosPlan?.payant && infosPlan.prix ? infosPlan.prix : 0);
  }, 0);
  ...
  useEffect(() => {
    const chargerDonneesPlateforme = async () => {
      ...
      const { data, error } = await supabase
        .from("etat_app")
        .select("id, donnees, code_entreprise, created_at");
      ...
      const lignes = data || [];
      setStats(calculerStatsPlateforme(lignes));
